 ...
 *
 * Testing Philosophy:
 * - One expectation per test (or consolidated with matchers)
 * - Test behavior, not implementation details
 * - Explicit describe blocks for each exported function
 * - Tests ordered to match implementation
 * - No loops, no deep nesting, no repetition
 *
...
describe('Dithering Module', () => {
  ...
  beforeAll(async () => {
    ...
    testImageBuffer = await new Promise((resolve, reject) => {
      const chunks = []
  ...
  describe('SUPPORTED_METHODS', () => {
    it('exports array of supported dithering methods', () => {
      expect(SUPPORTED_METHODS).toEqual(['floyd-steinberg', 'ordered', 'none'])
    })
  })
  ...
  describe('validateDitheringOptions', () => {
    ...
    it('falls back to defaults when invalid method provided', () => {
      const result = validateDitheringOptions({ method: 'invalid-method' })
