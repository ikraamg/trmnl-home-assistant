...
/**
 * NOTE: Integration tests use raw object literals that mirror form submissions.
 * We use type assertions since scheduleStore spreads input data at runtime.
 * The schedule store accepts these looser shapes and stores them as-is.
 */

/** Helper to create schedule input with type assertion */
const input = (data: Record<string, unknown>): ScheduleInput =>
  data as unknown as ScheduleInput

/** Helper to create schedule update with type assertion */
const update = (data: Record<string, unknown>): ScheduleUpdate =>
  data as unknown as ScheduleUpdate
...
describe('Schedule Store', () => {
  ...
  describe('createSchedule', () => {
    ...
    it('creates schedule with only required fields', async () => {
      const minimal = await createSchedule(
        TEST_SCHEDULE_FILE,
        input({
          name: 'Minimal',
          path: '/lovelace/0',
  ...
  describe('updateSchedule', () => {
    ...
    it('persists updates to file', async () => {
      ...
      await updateSchedule(TEST_SCHEDULE_FILE, created.id, update({ name: 'Updated' }))
