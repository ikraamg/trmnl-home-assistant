/**
 * Integration tests for webhook delivery (ENHANCED)
 *
 * Tests UploadToWebhookCommand with real HTTP server
 *
 * Testing Philosophy (from Alchemists.io):
 * - Integration tests use REAL HTTP server (WebhookTestServer fake)
 * - Spy pattern: verify requests AFTER they happen
 * - One expectation per test (consolidated with toMatchObject where appropriate)
 * - No loops, no deep nesting - explicit test cases
...
import { UploadToWebhookCommand } from '../../lib/scheduler/webhook-uploader.js'
...
describe('Webhook Integration', () => {
  ...
  const mockScreenshotFn = async (params) => {
  ...
  const uploadToWebhook = async (schedule, imageBuffer, format) => {
    const uploadCmd = new UploadToWebhookCommand(schedule, imageBuffer, format)
    try {
  ...
  afterEach(async () => {
    ...
    if (fs.existsSync(testOutputDir)) {
      ...
      files.forEach((file) => {
        if (file.match(/\.(png|jpeg|jpg|bmp)$/i)) {
  ...
  // ==========================================================================
  // UploadToWebhookCommand#call - Basic HTTP delivery
  // ==========================================================================

  describe('UploadToWebhookCommand#call', () => {
    ...
    it('sets Content-Type header based on image format', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/png')
      await uploadToWebhook(schedule, createPNGBuffer(), 'png')
    ...
    it('sends complete image buffer without corruption', async () => {
      ...
      const schedule = createWebhookSchedule('http://localhost:10002/integrity')
  ...
  describe('Content-Type Headers', () => {
    ...
    it('sends image/jpeg for JPEG format', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/jpeg')
    ...
    it('sends image/bmp for BMP format', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/bmp')
    ...
    it('does not allow webhook_headers to override Content-Type', async () => {
      const schedule = createWebhookSchedule(
        'http://localhost:10002/no-override',
        {
  ...
  describe('Custom Headers', () => {
    it('includes custom webhook headers in request', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/headers', {
    ...
    it('sends Authorization header with Bearer token', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/bearer', {
    ...
    it('sends multiple custom headers together', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/multi', {
        'X-Custom-1': 'value1',
        'X-Custom-2': 'value2',
        'X-Custom-3': 'value3',
      })
    ...
    it('sends webhook headers alongside Content-Type', async () => {
      const schedule = createWebhookSchedule('http://localhost:10002/auth', {
  ...
  describe('Response Handling', () => {
    it('handles 200 OK responses successfully', async () => {
      ...
      const schedule = createWebhookSchedule('http://localhost:10002/ok')
    ...
    it('handles 201 Created responses successfully', async () => {
      ...
      const schedule = createWebhookSchedule('http://localhost:10002/created')
    ...
    it('handles 204 No Content responses with empty body', async () => {
      ...
      const schedule = createWebhookSchedule(
        'http://localhost:10002/no-content'
      )
    ...
    it('handles large response bodies without errors', async () => {
      ...
      const schedule = createWebhookSchedule(
        'http://localhost:10002/large-body'
      )
  ...
  describe('Error Handling', () => {
    it('logs but does not throw on 400 client errors', async () => {
      ...
      const schedule = createWebhookSchedule(
        'http://localhost:10002/bad-request'
      )
    ...
    it('logs but does not throw on 404 not found', async () => {
      ...
      const schedule = createWebhookSchedule('http://localhost:10002/not-found')
    ...
    it('logs but does not throw on 500 server errors', async () => {
      ...
      const schedule = createWebhookSchedule('http://localhost:10002/error')
    ...
    it('logs but does not throw on connection refused', async () => {
      const schedule = createWebhookSchedule('http://localhost:19999/refused')
    ...
    it('logs but does not throw on DNS resolution failures', async () => {
      const schedule = createWebhookSchedule(
        'http://nonexistent.invalid.domain/test'
      )
    ...
    it('logs but does not throw on invalid URL schemes', async () => {
      const schedule = createWebhookSchedule('not-a-valid-url')
