...
function cleanCommitMessage(message) {
  return (
    message
      ...
      .replace(/^(feat|fix|change|refactor|update|add|improve)(\([^)]+\))?:\s*/i, '')
...
function formatChangelogEntries(categories) {
  ...
  if (categories.changed.length > 0) {
    sections.push('### Changed\n' + categories.changed.map((c) => `- ${c}`).join('\n'));
  }
...
function updateChangelog(newVersion, previousVersion) {
  ...
  let updatedRest;
  if (linksStart !== -1) {
    updatedRest = rest.slice(0, linksStart + 1) + newLink + rest.slice(linksStart);
  } else {
    updatedRest = rest + `\n${newLink}\n`;
  }
  ...
  console.log(
    `   Added: ${categories.added.length}, Changed: ${categories.changed.length}, Fixed: ${categories.fixed.length}, Other: ${categories.other.length}`
  );
}
...
function release(bumpType, options = {}) {
  ...
  if (push) {
    ...
    try {
      execSync(`gh release create v${newVersion} --title "v${newVersion}" --notes "${releaseNotes.replace(/"/g, '\\"')}"`, {
        stdio: 'inherit'
      });
      console.log('✅ GitHub release created - Docker images will build automatically');
    } catch (error) {
      console.error('⚠️  Failed to create GitHub release. Create manually with:');
